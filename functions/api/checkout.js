...
export async function onRequestPost({request,env}){
 try{
  ...
  const status=paymentMethod==='cash'?'cash_due':'pending';
  await env.DB.prepare('INSERT INTO orders(id,name,email,phone,delivery_date,items,total,status,created_at) VALUES(?,?,?,?,?,?,?,?,?)').bind(orderId,b.name,b.email,b.phone,b.deliveryDate,JSON.stringify(items),total,status,new Date().toISOString()).run();
